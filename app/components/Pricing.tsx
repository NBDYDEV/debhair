...
export default function Pricing() {
    ...
    return (
        <section className="py-12 sm:py-16 md:py-20 lg:py-24 bg-gradient-to-b from-gray-50 to-gray-100/50">
            <div className="container mx-auto px-4 sm:px-6 lg:px-8 max-w-[1600px]">
                ...
                <div className="hidden lg:block">
                    <div className="grid grid-cols-4 gap-5 xl:gap-8">
                        ...
                        {plans.map((plan) => (
                            <div
                                key={plan.id}
                                className={`flex flex-col rounded-[32px] xl:rounded-[40px] shadow-xl transition-all duration-300 hover:shadow-2xl overflow-hidden ${plan.isElite && 'bg-primarydark text-white'}`}
                            >
                                ...
                                <div className="p-6 xl:p-8">
                                    <Link
                                        href="/kapcsolat"
                                        className={`w-full py-4 xl:py-5 rounded-2xl font-bold text-sm uppercase tracking-wider transition-all duration-300 flex items-center justify-center gap-3 ${plan.isElite
                ...
                <div className="hidden md:block lg:hidden">
                    ...
                    <div className="text-center mb-10">
                        ...
                        <h2 className="text-4xl font-extrabold text-primarydark leading-tight mb-6">
                            Választható csomagok
                        </h2>
                        <div className="bg-white border border-gray-200 rounded-full p-1.5 inline-flex shadow-sm">
                            <button
                                onClick={() => setBillingPeriod('1nap')}
                                className={`px-6 py-2.5 rounded-full text-sm font-bold transition-all ${billingPeriod === '1nap' ? 'bg-primarydark text-white shadow-md' : 'text-gray-500'}`}
                            >
                                1 napos csomagok
                            </button>
                            <button
                                onClick={() => setBillingPeriod('2nap')}
                                className={`px-6 py-2.5 rounded-full text-sm font-bold transition-all ${billingPeriod === '2nap' ? 'bg-primarydark text-white shadow-md' : 'text-gray-500'}`}
                            >
                                2 napos csomagok
                            </button>
                        </div>
                    ...
                    {/* Tablet kártyák - 3 oszlop, desktop stílussal */}
                    <div className="grid grid-cols-3 gap-4">
                        {plans.map((plan) => (
                            <div
                                key={plan.id}
                                className={`flex flex-col rounded-[28px] shadow-xl transition-all duration-300 hover:shadow-2xl overflow-hidden ${plan.isElite && 'bg-primarydark text-white'}`}
                            >
                                {/* Fejléc */}
                                <div className={`h-[180px] p-5 text-center flex flex-col justify-center items-center border-b ${plan.isElite ? 'border-white/10' : 'border-gray-100'}`}>
                                    <h3 className="text-xl font-bold font-rem mb-1 flex justify-center items-center">
                                        {plan.isElite && <Gem className="w-8 h-8 text-secondary mr-2" />}
                                        {plan.name}
                                    </h3>
                                    <p className={`text-[10px] mb-3 font-medium uppercase tracking-wider ${plan.isElite ? 'text-white/60' : 'text-gray-400'}`}>
                                        {plan.hairCount}
                                    </p>
                                    <div className="flex items-baseline gap-1">
                                        <span className={`text-2xl font-extrabold ${plan.isElite ? 'text-white' : 'text-primarydark'}`}>
                                            {formatPrice(billingPeriod === '1nap' ? plan.onedayprice : plan.twodayprice)}
                                        </span>
                                        <span className={`text-sm font-bold ${plan.isElite ? 'text-white' : 'text-primarydark'}`}>Ft</span>
                                    </div>
                                    {plan.isElite && <div className='w-[110%] mt-4 border-t-2 border-dashed border-secondary'></div>}
                                </div>

                                {/* Features - fix magasságú sorok */}
                                <div className="px-4 flex-grow">
                                    {features.map((f, i) => (
                                        <div
                                            key={i}
                                            className={`h-[52px] flex items-center justify-center border-b last:border-0 ${plan.isElite ? 'border-white/10' : 'border-gray-300'}`}
                                        >
                                            {f[plan.id] ? (
                                                ...
                                                <div className="w-8 h-8 rounded-full bg-gray-100/30 flex items-center justify-center">
                                ...
                                <div className="p-4">
                                    <Link
                                        href="/kapcsolat"
                                        className={`w-full py-3.5 rounded-xl font-bold text-xs uppercase tracking-wider flex items-center justify-center gap-2 transition-all duration-300 ${plan.isElite
                                            ? 'bg-white text-primarydark hover:bg-secondary hover:scale-[1.02] shadow-lg'
                                            : 'bg-primarydark text-white hover:bg-primary'
                                            }`}
                                    >
                    ...
                    {/* Egyedi ajánlat */}
                    <div className="mt-8 p-5 bg-white rounded-2xl border border-gray-100 flex items-center justify-center gap-4 shadow-sm">
                        <div className="bg-primary/10 p-2.5 rounded-full">
                            <HelpCircle className="text-primary w-5 h-5" />
                        </div>
                        <p className="text-sm text-gray-600">
                            Nem találja a megfelelőt?{' '}
                            <Link href="/kapcsolat" className="text-primary font-bold hover:underline">
                                Kérjen egyedi ajánlatot!
                            </Link>
                        </p>
                    </div>
                ...
                <div className="md:hidden">
                    ...
                    <div className="mb-8">
                        ...
                        <h2 className="text-3xl font-extrabold text-primarydark leading-tight mb-5">
                            Választható csomagok
                        </h2>
                        <div className="bg-white border border-gray-200 rounded-full p-1 inline-flex shadow-sm">
                            <button
                                onClick={() => setBillingPeriod('1nap')}
                                className={`px-5 py-2.5 rounded-full text-xs font-bold transition-all ${billingPeriod === '1nap' ? 'bg-primarydark text-white' : 'text-gray-500'}`}
                            >
                                1 napos
                            </button>
                            <button
                                onClick={() => setBillingPeriod('2nap')}
                                className={`px-5 py-2.5 rounded-full text-xs font-bold transition-all ${billingPeriod === '2nap' ? 'bg-primarydark text-white' : 'text-gray-500'}`}
                            >
                                2 napos
                            </button>
                        </div>
                    ...
                    {/* Mobil kártyák - desktop stílussal */}
                    <div className="space-y-4">
                        {plans.map((plan) => (
                            <div
                                key={plan.id}
                                className={`rounded-2xl overflow-hidden shadow-lg transition-all ${plan.isElite && 'bg-primarydark'}`}
                            >
                                {/* Kártya fejléc - kattintható */}
                                <button
                                ...
                                >
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div className="text-left">
                                            <h3 className={`text-lg font-bold font-rem ${plan.isElite ? 'text-white' : 'text-primarydark'}`}>
                                                {plan.name}
                                            </h3>
                                    ...
                                    <div className="flex items-center gap-3">
                                        <div className="text-right">
                                            <span className={`text-xl font-extrabold ${plan.isElite ? 'text-white' : 'text-primarydark'}`}>
                                                {formatPrice(billingPeriod === '1nap' ? plan.onedayprice : plan.twodayprice)}
                                            </span>
                                            <span className={`text-sm ml-0.5 font-bold ${plan.isElite ? 'text-white' : 'text-primarydark'}`}>Ft</span>
                                        </div>
                                        <ChevronDown className={`w-5 h-5 transition-transform duration-300 ${openMobileCard === plan.id ? 'rotate-180' : ''} ${plan.isElite ? 'text-white/50' : 'text-gray-400'}`} />
                                    </div>
                                </button>

                                {/* Kinyitható tartalom */}
                                <div className={`overflow-hidden transition-all duration-300 ${openMobileCard === plan.id ? 'max-h-[600px]' : 'max-h-0'}`}>
                                    <div className={`px-5 pb-5 ${plan.isElite ? 'border-t-2 border-dashed border-secondary' : 'border-t border-gray-100'}`}>
                                        {/* Features - fix magasságú sorok */}
                                        <div className="py-2">
                                            {features.map((f, i) => (
                                                <div key={i} className={`h-[48px] flex items-center justify-between border-b last:border-0 ${plan.isElite ? 'border-white/10' : 'border-gray-200'}`}>
                                                        ...
                                                        {f.tooltip && (
                                                            <div className="group relative">
                                                                ...
                                                                <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 px-3 py-2 bg-primarydark text-white text-xs rounded-lg shadow-xl opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 w-56 z-50 leading-relaxed">
                                                                    {f.tooltip}
                                                                    <div className="absolute top-full left-1/2 -translate-x-1/2 border-4 border-transparent border-t-primarydark"></div>
                                                                </div>
                                                            </div>
                                                        )}
                                                    </div>
                                        ...
                                        <Link
                                            href="/kapcsolat"
                                            className={`w-full py-4 rounded-xl font-bold text-sm uppercase tracking-wider flex items-center justify-center gap-2 transition-all mt-2 ${plan.isElite
                                                ? 'bg-white text-primarydark hover:bg-secondary hover:scale-[1.02] shadow-lg'
                                                : 'bg-primarydark text-white hover:bg-primary'
                                                }`}
                                        >
                    ...
                    {/* Egyedi ajánlat */}
                    <div className="mt-8 p-5 bg-white rounded-2xl border border-gray-100 flex items-start gap-4 shadow-sm">
                        <div className="bg-primary/10 p-2.5 rounded-full flex-shrink-0">
                            <HelpCircle className="text-primary w-5 h-5" />
                        </div>
                        <div>
                            ...
                            <Link href="/kapcsolat" className="text-primary font-bold text-sm hover:underline">
                                Kérjen egyedi ajánlatot →
                            </Link>
                        </div>
                    </div>
